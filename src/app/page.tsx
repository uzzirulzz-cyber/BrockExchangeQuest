...
import { MessagesView } from "@/components/blockexchange/messages-view";
  ...
  return (
    <div className="min-h-screen flex flex-col">
      ...
      <div className="flex-1 flex flex-col lg:flex-row pb-16 lg:pb-0">
        {/* Desktop sidebar — only for logged-in customers on their dashboard views */}
        {user && user.role === "CUSTOMER" && !isAuthView && view !== "home" && view !== "wallet" && <CustomerSidebar />}
      ...
      <PWAInstallPrompt />
      <PasswordChangeModal />
      {/* Mobile bottom tab bar — hidden on auth views + home (marketing) + desktop */}
      {user && user.role === "CUSTOMER" && !isAuthView && <MobileTabBar />}
    </div>
  );
}
